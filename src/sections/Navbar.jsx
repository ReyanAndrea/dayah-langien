  ...
  return (
    <nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled
        ? 'bg-navy-900/90 backdrop-blur-md shadow-lg shadow-black/20 py-3'
        : 'bg-transparent py-5'
        }`}
    >
        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="text-sm font-medium text-navy-100 hover:text-gold-500 transition-colors"
            >
              {link.label}
            </a>
          ))}
